resource "aws_cloudfront_distribution" "roboboshop" {
  origin {
  ...
  ordered_cache_behavior {
    ...
    viewer_protocol_policy = "hhtps-only"
    cache_policy_id = local.cachingOptimized
  }
...
resource "aws_route53_record" "www" {
  ...
  #CDN details 
  alias {
    name                   = aws_cloudfront_distribution.roboboshop.domain_name
    zone_id                = aws_cloudfront_distribution.roboboshop.hosted_zone_id
