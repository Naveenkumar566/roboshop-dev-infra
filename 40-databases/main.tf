...
resource "terraform_data" "mongodb" {
  ...
  provisioner "remote-exec" {
    inline = [
        ...
        "sudo sh /tmp/ansible.sh mongodb ${var.environment}"
    ]
  }
}
...
resource "terraform_data" "ansible_redis" {
  triggers_replace = [
  ...
  provisioner "remote-exec" {
    inline = [
        "chmod +x /tmp/ansible.sh",
        "sudo sh /tmp/ansible.sh redis ${var.environment}"
    ]
  }
}
...
resource "terraform_data" "mysql" {
  ...
  provisioner "remote-exec" {
    inline = [
        "chmod +x /tmp/ansible.sh",
        "sudo sh /tmp/ansible.sh mysql ${var.environment}"
    ]
  }
}
...
resource "terraform_data" "rabbitmq" {
  ...
  provisioner "remote-exec" {
    inline = [
        "chmod +x /tmp/ansible.sh",
        "sudo sh /tmp/ansible.sh rabbitmq ${var.environment}"
    ]
  }
}